...
public:
  ...
  bool truncatableCheck(int num);		//Checks if a num is a truncatable prime (there is only 11, primes less then 10 does not count), example 3797, 797, 97, 7, 3797, 379, 79, 3
...
bool TestPrime::truncatableCheck(int num)			//Checks if number is a truncatable prime
{
  int num2;						//num2 is num with some other digit as first digit
  bool isPrime;						//if tested number is prime
  int factor;						//factor for truncation
  
  isPrime=true;					//num is prime until proven otherwise
  num2=num;
  factor=1;
  while(num2>0 && isPrime)
  {
    isPrime = isPrime && this->isPrime(num2);	//check if num, num/10, num/100... is prime
    num2/=10;
    factor*=10;					//factor for next step is increased
  }
  num2=num;
  factor/=10;					//factor is dependent on numbers of digits in num
  while(num2>0 && isPrime)
  {
    isPrime = isPrime && this->isPrime(num2);
    num2-=(num2/factor)*factor;			//remove first digit in num
    factor/=10;
  }
...
int main(int argc, char **argv) {
  TestPrime testPrime;
  int truncatablePrimesCount=0;				//number of circular primes
  int truncatablePrimesSum=0;				//Sum of all truncatable primes
  int i=10;						//number to be checked
  
  while(truncatablePrimesCount < 11)
  {
    if(testPrime.truncatableCheck(i))	//test if digits is circular prime
    {
      truncatablePrimesCount++;
      std::cout << "Truncatable prime " << truncatablePrimesCount << " found: " << i << std::endl;
      truncatablePrimesSum+=i;
    }
    i++;
  }
  std::cout << std::endl << "Sum of truncatable primes: " << truncatablePrimesSum << std::endl;
  return 0;
}