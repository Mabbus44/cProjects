  ...
  while(it != nValuesComplete.end()){
    ...
    if(*it%2==0){
      if(*it <= maxSafenValue/4){
        ...
        std::cout << "Inserted " << *it << " * 4 = " << *it*4 << std::endl;
      }
    }
    primeIt++;
    ...
    while(primeIt!=primes.end() && *it<=maxSafenValue/primeSquare){
      ...
      std::cout << "Inserted " << *it << " * " << (*primeIt) << " * " << (*primeIt) << " = " << *it*primeSquare << std::endl;
      primeIt++;
...
int main(){
  ...
  Euler66 e;
  e.maxSafenValue=1000000;//int64Max/100;
