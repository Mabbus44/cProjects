...
Number Number::operator+(const Number& number){
  ...
  size=max(digitCount, number.digitCount)+2;			//Calculate new size
...
Number Number::operator*(const Number& number){
  ...
  for(int i=1; i<digitCount+1; i++){				//All digits of first number
    for(int i2=1; i2<number.digitCount+1; i2++){		//All digits of second number
      cout << "Digit " << i+i2-1 << " = " << (int)result.digits[i+i2-1] << " + " << (int)digits[i] << " * " << (int)number.digits[i2] << " + " << rest << endl;
      result.digits[i+i2-1] += digits[i] * number.digits[i2] + rest;	//Multiply 2 digits, add product + sum to result digit
      cout << "Digit " << i+i2-1 << " = " << (int)result.digits[i+i2-1] << endl;
      rest = 0;							//Reset rest
    ...
    cout << "Digit " << i+number.digitCount << " = " << (int)result.digits[i+number.digitCount] << " + " << rest << endl;
    ...
    cout << "Digit " << i+number.digitCount-1 << " = " << (int)result.digits[i+number.digitCount-1] << endl;
  }
