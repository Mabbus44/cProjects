...
void Leaf::insert(__int64 value){
  if(value<this->value){
    if(left)
      left->insert(value);
    else
...
Leaf* Leaf::search(__int64 value){
  if(value == this->value)
    return this;
  if(value < this->value)
  {
...
bool Leaf::search(__int64 minVal, __int64 maxVal){
  if(value > maxVal){
    if(!left)
      return false;
    return left->search(minVal, maxVal);
  }
  if(value < minVal){
    if(!right)
      return false;
    return right->search(minVal, maxVal);
  }
  return true;
}
...
void Btree::insert(__int64 value){
  if(root)
    root->insert(value);
  else
    root = new Leaf(value);
...
Leaf* Btree::search(__int64 value){
  if(!root)
    return nullptr;
  return root->search(value);
}

bool Btree::search(__int64 minVal, __int64 maxVal){
  if(!root)
    return false;
  return root->search(minVal, maxVal);
}
